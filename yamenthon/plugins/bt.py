    ...
    try:
        async with bot.conversation(chat) as conv:
            try:
                ...
                filename = await conv.send_message("YamenThon")
...
@zthon.on(admin_cmd(pattern="تيك توك(?: |$)(.*)"))
async def _(iqthon):
    reply_message = await iqthon.get_reply_message()
...
@zthon.on(admin_cmd(pattern="زخرفه_عربي ?(.*)"))
async def _(event):
...
@zthon.on(admin_cmd(pattern="زخرفه_انكليزي ?(.*)"))
async def _(event):
    input_str = event.pattern_match.group(1)
    reply_to_id = await reply_id(event)
    if event.reply_to_msg_id:
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response.text.startswith("رجاء قم بالرد على الكلمه التي تريد زخرفتها "):
            ...
            await catevent.delete()
            await event.client.send_message(event.chat_id, response.message)
@zthon.on(admin_cmd(pattern="انستا (.*)"))
...
@zthon.on(admin_cmd(pattern="هديه ?(.*)"))
async def iq(iqthon):
    ...
    await l5[0].click(iqthon.chat_id)
    await iqthon.delete()
@zthon.on(admin_cmd(pattern="كشف الفايروسات( -i)?$"))    
...
@zthon.on(admin_cmd(pattern="تقويم ([\s\S]*)"))    
async def _iq(iqthon):
    input_str = iqthon.pattern_match.group(1)
...
# ضل راقب ولابسك 🤣
@zthon.on(admin_cmd(pattern="سؤال ?(.*)"))
async def _(event):
    input_str = event.pattern_match.group(1)
    reply_to_id = await reply_id(event)
    if event.reply_to_msg_id:
