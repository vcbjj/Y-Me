...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "بالـرد ع اي ملف (يدعم سورس يمنثون) لـ تنصيبه في بوتك.",
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await edit_delete(
                    event,
                    f"**    ⃟⁞⃟⟢ ╎تـم تنصـيب المـلف** `{os.path.basename(downloaded_file_name)}` **.. بـ نجـاح ☑️**",
                    10,
                )
            else:
...
@zedub.zed_cmd(
    ...
    info={
        "header": "لـ تحميـل اي ملف مـجددًا .. اذا كنت قد الغيت تحميله مسبقـاً",
...
async def load(event):
    ...
    try:
        ...
        await edit_delete(event, f"**    ⃟⁞⃟⟢ ╎تـم تحميـل المـلف** {shortname} **.. بـ نجـاح ☑️**", 10)
    except Exception as e:
        await edit_or_reply(
            event,
...
async def unload(event):
    ...
    try:
        remove_plugin(shortname)
        await edit_or_reply(event, f"**    ⃟⁞⃟⟢ ╎تم الغـاء تحميـل** {shortname} **.. بنجـاح✓**")
    except Exception as e:
        await edit_or_reply(event, f"**    ⃟⁞⃟⟢ ╎تم الغـاء تحميـل** {shortname} **.. بنجـاح✓**\n{e}")
...
async def unload(event):
    ...
    try:
        remove_plugin(shortname)
        await edit_or_reply(event, f"**    ⃟⁞⃟⟢ ╎تـم الغـاء تنصيب المـلف** {shortname} **.. بـ نجـاح ☑️**")
    except Exception as e:
        await edit_or_reply(event, f"**    ⃟⁞⃟⟢ ╎تـم الغـاء تنصيب المـلف** {shortname} **.. بـ نجـاح ☑️**\n{e}")
