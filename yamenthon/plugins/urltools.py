...
async def _(event):
    ...
    if not input_str:
        return await edit_delete(
            event, "**    ⃟⁞⃟⟢ ╎بالـرد ع رابـط او باضافـة رابـط مع الامـر ...**", 10
        )
    check = url(input_str)
    if not check:
        ...
        check = url(catstr)
    if not check:
        return await edit_delete(event, "**    ⃟⁞⃟⟢ ╎عـذراً .. هـذا الرابـط غيـر مدعـوم ؟!**", 10)
    sample_url = f"https://da.gd/dns/{input_str}"
    if response_api := requests.get(sample_url).text:
        await edit_or_reply(event, f"**    ⃟⁞⃟⟢ ╎الدوميـن الخـاص بالرابـط** {input_str} \n**    ⃟⁞⃟⟢ ╎هـو :** \n{response_api}")
    else:
        await edit_or_reply(
            event, f"**    ⃟⁞⃟⟢ ╎عـذراً .. لا يمكنني العثـور علـى دوميـن الرابـط** {input_str} **على الشبكـة العنكبوتيـه**"
        )
...
async def _(event):
    ...
    if response_api := requests.get(sample_url).text:
        await edit_or_reply(
            event, f"**    ⃟⁞⃟⟢ ╎الرابـط المختصر** {response_api} \n**    ⃟⁞⃟⟢ ╎الرابـط** {input_str} \n**    ⃟⁞⃟⟢ ╎تم انشـاء الإختصـار .. بنجـاح**", link_preview=False
        )
    else:
        await edit_or_reply(event, "**    ⃟⁞⃟⟢ ╎خـطأ بالاختصـار .. الرجـاء المحاولـة لاحقـاً**")
...
async def _(event):
    ...
    if str(r.status_code).startswith("3"):
        await edit_or_reply(
            event,
            f"**    ⃟⁞⃟⟢ ╎الرابـط المختصر :** {input_str}\n**    ⃟⁞⃟⟢ ╎الرابـط الاصـلي :** {r.headers['Location']}",
            link_preview=False,
        )
    else:
        await edit_or_reply(
