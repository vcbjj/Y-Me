...
# =========================================================== #
#                           الملـــفـ كتـــابـــة مـــن الصفــــر - t.me/YamenThon                           #
# =========================================================== #
Warn = "hhh"
ZEDTHON_BEST_SOURCE = "[ᯓ 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - اذاعـة خـاص 🚹](t.me/YamenThon) .\n\n**- جـارِ الاذاعـه خـاص لـ أعضـاء الكـروب 🛗\n- الرجـاء الانتظـار .. لحظـات ⏳**"
...
async def malath(event):
    ...
    if yamen:
        await edit_or_reply(event, "**    ⃟⁞⃟⟢ ╎بالـࢪد ؏ــلى ࢪســالة أو وسـائـط**")
        return
    elif event.is_reply:
        resources = await event.get_reply_message()
    else:
        await edit_or_reply(event, "**    ⃟⁞⃟⟢ ╎بالـࢪد ؏ــلى ࢪســالة أو وسـائـط**")
        return
    ...
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if resources.text:
            try:
                await borg.send_message(magtxt, resources, link_preview=False)
                success += 1
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - اذاعـة خـاص 🚹](t.me/YamenThon) .\n\n**    ⃟⁞⃟⟢ ╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**    ⃟⁞⃟⟢ ╎عـدد {success} عضـو**"
...
async def unmalath(event):
    if not event.chat_id in spam_chats:
        ...
        try:
            ...
            pass
        return await event.edit("**    ⃟⁞⃟⟢ ╎تم إيقـافـ عمليـة الاذاعـه للأعضـاء هنـا .. بنجـاح✓**")
        ...
        if resources.text:
            try:
                ...
                if user_entity.bot or user_entity.deleted:
                    continue
                await zedub.send_message(user_entity.id, resources, link_preview=False)
                success += 1
            except UserAdminInvalidError:
                pass
            except Exception as e:
                zzz.edit(f"خطـأ فـي إرسـال الرسـالة إلــى {user_entity.id}: {str(e)}")
        elif resources.media:
            try:
                user_entity = await zedub.get_entity(user)
                if user_entity.bot or user_entity.deleted:
                    continue
                await zedub.send_file(user_entity.id, resources.media, caption=resources.text)
                success += 1
            except UserAdminInvalidError:
                pass
            except Exception as e:
                zzz.edit(f"خطـأ فـي إرسـال الرسـالة إلــى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - اذاعـة زاجـل 🕊](t.me/YamenThon) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**    ⃟⁞⃟⟢ ╎تمت الاذاعـه .. بنجـاح  ✅**\n**    ⃟⁞⃟⟢ ╎عـدد {success} أشخـاص**"
