...
async def zedgban(event):  # sourcery no-metrics
    ...
    if user.id == zedub.uid:
        return await edit_delete(zede, "**    ⃟⁞⃟⟢ ╎عـذراً ..لا استطيـع حظـࢪ نفسـي **")
    if user.id in zel_dev:
        return await edit_delete(zede, "**    ⃟⁞⃟⟢ ╎عـذراً ..لا استطيـع حظـࢪ احـد المطـورين عـام **")
    if user.id == 5571722913 or user.id == 5571722913:
        return await edit_delete(zede, "**    ⃟⁞⃟⟢ ╎عـذراً ..لا استطيـع حظـࢪ مطـور السـورس عـام **")


    if gban_sql.is_gbanned(user.id):
        await zede.edit(
            f"**    ⃟⁞⃟⟢ ╎المسـتخـدم ↠** [{user.first_name}](tg://user?id={user.id}) \n**    ⃟⁞⃟⟢ ╎مـوجــود بالفعــل فـي ↠ قائمـة المحظــورين عــام**"
        )
    else:
    ...
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(zede, "**    ⃟⁞⃟⟢ ╎عــذراً .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـده ع الأقــل **")
    await zede.edit(
        f"**    ⃟⁞⃟⟢ ╎جـاري بـدء حظـر ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**    ⃟⁞⃟⟢ ╎مـن ↠ {len(san)} كــروب**"
    )
    for i in range(sandy):
        try:
            ...
            await event.client.send_message(
                BOTLOG_CHATID,
                f"**    ⃟⁞⃟⟢ ╎عــذراً .. لـيس لـديــك صـلاحيـات فـي ↠**\n**    ⃟⁞⃟⟢ ╎كــروب :** {get_display_name(achat)}(`{achat.id}`)",
            )
    end = datetime.now()
    zedtaken = (end - start).seconds
    if reason:
        await zede.edit(
            f"**    ⃟⁞⃟⟢ ╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**    ⃟⁞⃟⟢ ╎تم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zedtaken} ثـانيـه**\n**    ⃟⁞⃟⟢ ╎السـبب :** {reason}"
        )
    else:
        await zede.edit(
...
async def zedgban(event):
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        return await edit_delete(
            zede,
            f"**    ⃟⁞⃟⟢ ╎المسـتخـدم ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**    ⃟⁞⃟⟢ ╎ليـس مـوجــود فـي ↠ قائمـة المحظــورين عــام**",
        )
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(zede, "**    ⃟⁞⃟⟢ ╎عــذراً .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـده ع الأقــل **")
    await zede.edit(
        f"**    ⃟⁞⃟⟢ ╎جـاري الغــاء حظـر ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**    ⃟⁞⃟⟢ ╎مـن ↠ {len(san)} كــروب**"
    )
    for i in range(sandy):
        try:
    ...
    if reason:
        await zede.edit(
            f"**    ⃟⁞⃟⟢ ╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**    ⃟⁞⃟⟢ ╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zedtaken} ثـانيـه**\n**    ⃟⁞⃟⟢ ╎السـبب :** {reason}"
        )
    else:
        await zede.edit(
            f"**    ⃟⁞⃟⟢ ╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**    ⃟⁞⃟⟢ ╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zedtaken} ثـانيـه**"
...
async def gablist(event):
    ...
    if len(gbanned_users) > 0:
        for a_user in gbanned_users:
            if a_user.reason:
                GBANNED_LIST += f"**    ⃟⁞⃟⟢ ╎المستخـدم :**  [{a_user.chat_id}](tg://user?id={a_user.chat_id}) \n**    ⃟⁞⃟⟢ ╎سـبب الحظـر : {a_user.reason} ** \n\n"
            else:
                GBANNED_LIST += (
                    f"**    ⃟⁞⃟⟢ ╎المستخـدم :**  [{a_user.chat_id}](tg://user?id={a_user.chat_id}) \n**    ⃟⁞⃟⟢ ╎سـبب الحظـر : لا يـوجـد ** \n\n"
                )
    else:
...
async def startgmute(event):
    ...
    if is_muted(userid, "gmute"):
        return await edit_or_reply(
            event,
            f"**    ⃟⁞⃟⟢ ╎المستخـدم**  {_format.mentionuser(user.first_name ,user.id)} \n**    ⃟⁞⃟⟢ ╎مڪتوم سابقـاً**",
        )
    try:
        mute(userid, "gmute")
...
async def endgmute(event):
    ...
    if not is_muted(userid, "gmute"):
        return await edit_or_reply(
            event, f"**    ⃟⁞⃟⟢ ╎المستخـدم :** {_format.mentionuser(user.first_name ,user.id)}\n\n**    ⃟⁞⃟⟢ ╎غيـر مكتـوم عــام ✓**"
        )
    try:
        unmute(userid, "gmute")
    except Exception as e:
...
async def catgkick(event):  # sourcery no-metrics
    ...
    if sandy == 0:
        return await edit_delete(zede, "**    ⃟⁞⃟⟢ ╎عــذراً .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـده ع الأقــل **")
    await zede.edit(
        f"**    ⃟⁞⃟⟢ ╎بـدء طـرد ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**    ⃟⁞⃟⟢ ╎فـي ↠ {len(san)} كــروب**"
    )
    for i in range(sandy):
        try:
