...
async def echo(event):
    ...
    if is_echo(chat_id, user_id):
        return await edit_or_reply(event, "**    ⃟⁞⃟⟢ ╎تم تفعيل الازعاج على الشخص .. بنجاح✓**")
    try:
        addecho(chat_id, user_id, chat_name, user_name, user_username, chat_type)
    except Exception as e:
        await edit_delete(zedubevent, f"**    ⃟⁞⃟⟢ ╎خطـأ :**\n`{str(e)}`")
    else:
        await edit_or_reply(
            zedubevent,
...
async def echo(event):
    ...
    if is_echo(chat_id, user_id):
        try:
            ...
            await edit_or_reply(event, "**    ⃟⁞⃟⟢ ╎تم ايقاف التقليد لهذا المستخدم**")
    else:
...
async def echo(event):
    ...
    if input_str:
        ...
        try:
            ...
            await edit_delete(event, f"**    ⃟⁞⃟⟢ ╎خطـأ :**\n`{str(e)}`", 10)
        else:
            await edit_or_reply(
                event, "**    ⃟⁞⃟⟢ ╎تم حذف تقليد جميع المستخدمين في جميع الدردشات.**"
            )
    else:
...
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
    output_str = "**    ⃟⁞⃟⟢ ╎قائمـة الاشخـاص المقلـدهـم:\n\n**"
    if input_str:
        lsts = get_all_echos()
        group_chats = ""
        if len(lsts) <= 0:
        ...
        output_str = "**    ⃟⁞⃟⟢ ╎الاشخـاص المقلـدهـم في هذه الدردشـه :\n**" + private_chats
