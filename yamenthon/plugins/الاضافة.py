...
async def get_users(event):
    ...
    if not sender.id == me.id:
        await zedb.edit("**    ⃟⁞⃟⟢ ╎ جـارِ إتمـام العمليـة انتظــر ⅏ . . .**")
    else:
        await zedb.edit("**    ⃟⁞⃟⟢ ╎ جـارِ إتمـام العمليـة انتظــر ⅏ . . .**")
    if event.is_private:
    ...
    await zedb.edit("**    ⃟⁞⃟⟢ ╎حالة الإضافة:**\n\n**    ⃟⁞⃟⟢ ╎تتم جمع معلومات المستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(chat):
        try:
            ...
            await asyncio.sleep(random.randint(7, 15))  # تأخير عشوائي بين 10 - 30 ثانية
            await zedb.edit(f"**    ⃟⁞⃟⟢ ╎تتم الإضافة **\n\n• إضيف `{s}` \n•  خطأ بإضافة `{f}` \n\n**× اخر خطأ:** `{error}`")
        except Exception as e:
            error = str(e)
            f = f + 1
    return await zedb.edit(f"**    ⃟⁞⃟⟢ ╎اڪتملت الإضافة ✅** \n\n• تم بنجاح إضافة `{s}` \n• خطأ بإضافة `{f}`")
