...
async def _(event):
    ...
    if input_str := event.pattern_match.group(2):
        ...
        try:
            if p.first_name:
                return await edit_or_reply(
                    event, f"**    ⃟⁞⃟⟢ ╎ايـدي المستخـدم**  `{input_str}` **هـو** `{p.id}`"
                )
        except Exception:
            try:
                if p.title:
                    return await edit_or_reply(
                        event, f"**    ⃟⁞⃟⟢ ╎ايـدي المستخـدم**  `{p.title}` **هـو** `{p.id}`"
                    )
            except Exception as e:
                LOGS.info(str(e))
        await edit_or_reply(event, "**    ⃟⁞⃟⟢ ╎أدخل إما اسم مستخدم أو الرد على المستخدم**")
        ...
        if r_msg.media:
            ...
            await edit_or_reply(
                event,
                f"**    ⃟⁞⃟⟢ ╎ايـدي الدردشـه : **`{event.chat_id}`\n\n**    ⃟⁞⃟⟢ ╎ايـدي المستخـدم : **`{r_msg.sender_id}`\n\n**    ⃟⁞⃟⟢ ╎ايـدي الميديـا : **`{bot_api_file_id}`",
            )
        ...
        await edit_or_reply(event, f"**    ⃟⁞⃟⟢ ╎ايـدي الدردشـه : **`{event.chat_id}`")
