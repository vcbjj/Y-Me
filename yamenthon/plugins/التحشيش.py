...
@zedub.zed_cmd(pattern="مسح الحساب")
async def _(zed):
    await edit_or_reply(
        zed,
        "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/YamenThon) 🗑♻️𓆪\n**𓍹━─━─━─━─𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ",
    )
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(
            mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين المساعديـن  ❏╰**"
        )
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/YamenThon) 𓆪\n\n"
...
async def zed(mention):
    ...
    await edit_or_reply(
        mention,
        f"**🚺 ╎ المستخـدم ه ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـها مـزة الكروب .. بنجـاح 🥳💃.** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ",
    )
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(
            mention, f"**╮ ❐ لك هـذا مطـور . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**"
        )
    if user.id == 5571722913:
        return await edit_or_reply(
            mention, f"**╮ ❐ لك هـذا زلـزال الهيبـه . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**"
        )
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
