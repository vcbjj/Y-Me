...
async def monito_p_m_s(event):  # sourcery no-metrics
    ...
    if not sender.bot:
        ...
        fullname = f"{sender.first_name}{sender.last_name}" if sender.last_name else sender.first_name #Write Code By T.me/YamenThon 
        user_name = f"@{sender.username}" if sender.username else "لا يوجـد" #Write Code By T.me/YamenThon 
