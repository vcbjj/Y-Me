...
from jmrobot.core.managers import edit_or_reply
...
async def fetch_info(replied_user, event):
    ...
    rozrtba = (
        ".「  مطـور السورس  」."
        if user_id == 5571722913 or user_id == 5571722913 or user_id == 5571722913
        else (".「  العضـو  」.")
    )
    rozrtba = (
        ".「 مـالك الحساب  」."
        if user_id == (await event.client.get_me()).id
        and user_id != 5571722913
        and user_id != 5571722913
        and user_id != 5571722913
        else rozrtba
    )
    ...
    return photo, caption@jmrobot.ar_cmd(pattern="ايدي(?: |$)(.*)")
async def who(event):
...
@jmrobot.ar_cmd(pattern="رابط الحساب(?:\s|$)(\s\S]*)")
async def permalink(mention):
    ...
    await edit_or_reply(mention, f"⪼  {tag}  𓆰. ")
