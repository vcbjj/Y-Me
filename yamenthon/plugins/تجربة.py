...
async def hi(event):
    await edit_or_reply(
        event,
        "**[ᯓ 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - تنفيــذ الاكــواد](t.me/YamenThon)\n⋆─┄─┄─┄──┄─┄─┄─⋆**\n\n الامر: `.تجربه` + كود برمجي\n- يقوم بتشغيل الكود و إظهار النتيجة",
        link_preview=False,
    )
...
async def _(event):
    ...
    yamen = await edit_or_reply(event, "**- جار تنفيـذ الكـود .. انتظـر**")
    ...
    await edit_or_reply(
        ...
        text=final_output,
        aslink=True,
        linktext=f"**    ⃟⁞⃟⟢ ╎الكــود : **\n```{cmd}``` \n\n**    ⃟⁞⃟⟢ ╎النتيجـة : **\n",
    )
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"**    ⃟⁞⃟⟢ ╎تم تشغيـل وتجـربـة امـر {cmd} .. بنجـاح**"
        )
