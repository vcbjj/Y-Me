...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"**    ⃟⁞⃟⟢ ╎ تم إنشاء حساب تيليجـراف جديد {auth_url} للدورة الحالية‌‌** \n**    ⃟⁞⃟⟢ ╎ لا تعطي عنوان الرابـط هـذا لأي شـخص**",
        )
    ...
    if input_str in ["ميديا", "m"]:
        ...
        try:
            ...
            os.remove(downloaded_file_name)
            await zedevent.edit(
                f"**    ⃟⁞⃟⟢ ╎الــرابـط : ** [اضغــط هنـــا](https://graph.org{media_urls[0]})\
                    \n**    ⃟⁞⃟⟢ ╎الـوقـت : **`{ms} seconds.`",
                link_preview=True,
            )
