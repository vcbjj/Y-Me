...
async def parse_ocr_space_api(event):
    if event.fwd_from:
        return
    await event.edit("**   ⃟⁞⃟⟢╎جـارِ جلب النص من الميديـا ▬▭ ...**")
...
async def ocr(event):
    ...
    zevent = await edit_or_reply(event, "**   ⃟⁞⃟⟢╎جـارِ جلب النص من الميديـا ▬▭ ...**")
    if not os.path.isdir(Config.TEMP_DIR):
        os.makedirs(Config.TEMP_DIR)
