...
from yamenthon import Convert, zedub
...
async def maccmd(event):  # sourcery no-metrics
    ...
    if not reply or not mediatype or mediatype not in ["Photo", "Sticker"]:
        return await edit_delete(event, "**   ⃟⁞⃟⟢╎بالـرد ع صورة او ملصق لعمل فرام ..**")
    if mediatype == "Sticker" and reply.document.mime_type == "application/i-tgsticker":
        return await edit_delete(
            event,
            "**   ⃟⁞⃟⟢╎بالـرد ع صورة او ملصق لعمل فرام**\n**   ⃟⁞⃟⟢╎الملصقات المتحركه غير مدعومه ...!!**",
        )
    catevent = await event.edit("**   ⃟⁞⃟⟢╎جـارِ عمل فرام لملف الميديا ...**")
    ...
    try:
        ...
        if imag[1] is None:
            return await edit_delete(
                imag[0], "**   ⃟⁞⃟⟢╎عـذراً .. لم استطـع استخـراج صـوره من ملـف الميـديا هـذا ؟!**"
            )
        image = Image.open(imag[1])
    except Exception as e:
        return await edit_delete(catevent, f"**   ⃟⁞⃟⟢╎خطـأ في الصـورة :**\n__{e}__")
    wid, hgt = image.size
...
async def memes(event):
    ...
    if not reply:
        return await edit_delete(event, "**   ⃟⁞⃟⟢╎قم بالـرد ع وسـائط مدعومـه ...**")
    catid = await reply_id(event)
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not zedinput:
        return await edit_delete(
            event, "**   ⃟⁞⃟⟢╎حقوق + نص بالـرد ع ملف ميديا مدعوم ...**"
        )
    if ";" in zedinput:
    ...
    if output[1] is None:
        return await edit_delete(
            output[0], "**   ⃟⁞⃟⟢╎عـذراً .. لم استطـع استخـراج صـوره من ملـف الميـديا هـذا ؟!**"
        )
...
async def memes(event):
    ...
    if not (reply and (reply.media)):
        await edit_or_reply(event, "**   ⃟⁞⃟⟢╎قم بالـرد ع وسـائط مدعومـه ...**")
        return
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    catid = await reply_id(event)
    if not os.path.isdir("./temp/"):
...
async def memes(event):
    ...
    try:
        ...
        return await edit_delete(event, f"**   ⃟⁞⃟⟢╎خطـأ :**\n`{e}`")
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**   ⃟⁞⃟⟢╎قم بالـرد ع وسـائط مدعومـه ...**")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
